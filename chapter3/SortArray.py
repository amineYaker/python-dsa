class SortArray(object):
    ## A simple array class that supports three sorting algorithms: bubble sort, selection sort, and insertion sort.
    ...
    def insertionSort(self):  # sorts by repeated inserts
        for outer in range(1, self.__nItems):  # mark one element
            ...
            while (
                inner > 0 and temp < self.__a[inner - 1]
            ):  # Marked elem is smaller so we shift to the right
                self.__a[inner] = self.__a[inner - 1]
                inner -= 1
            self.__a[inner] = (
                temp  # marked elem is no longer smaller we put it in the "hole"
            )
