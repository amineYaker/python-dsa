        ...
        if delim:  # delimiter
            if token == "(":
                ...
                while not s.isEmpty():
                    top = (
                        s.pop()
                    )  # popping items from the stack until we find the opening one
                    if top == "(":
                        break
            ...
            while not s.isEmpty():
                ...
                if top == "(" or precedence(top) < prec:
                    s.push(
                        top
                    )  # we push it back because it is lower operator or the opening parenthesis of the current expression
                    break
